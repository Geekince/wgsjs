...
import static com.wwsl.wgsj.Constants.GIF_CAR_PREFIX;
...
public class LiveEnterRoomAnimPresenter {
    ...
    private void startAnim(LiveEnterRoomBean bean) {
        ...
        if (u != null && liveChatBean != null) {
            ...
            if (u.getVip() != 0 || liveChatBean.getGuardType() != Constants.GUARD_TYPE_NONE) {
                needAnim = true;
            ...
            if (car != null && car.getId() != 0) {
                ...
                if (!TextUtils.isEmpty(url)) {
                    needAnim = true;
                    mShowGif = true;
                    mWordText.setText(u.getUsername() + car.getWords());
                    GifCacheUtil.getFile(GIF_CAR_PREFIX + car.getId(), url, mDownloadGifCallback);
                }
            }
            if (!needAnim) {
                getNextEnterRoom();
