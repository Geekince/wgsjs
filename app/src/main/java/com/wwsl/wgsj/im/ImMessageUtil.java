...
import com.wwsl.wgsj.bean.RedPacketNetBean;
import com.wwsl.wgsj.bean.UserBean;
import com.wwsl.wgsj.bean.VideoBean;
import com.wwsl.wgsj.custom.MyImageView;
...
public class ImMessageUtil {
    ...
    private ImMessageUtil() {
        ...
        mSimpleDateFormat = new SimpleDateFormat("MM-dd HH:mm", Locale.CHINA);
    ...
    public void init() {
        ...
        LogUtils.e(TAG, "notificationFlag: " + notificationFlag);
    ...
    public void loginJMessage(String uid) {
        String imUid = getImUid(uid);
        JMessageClient.login(imUid, getPwd(imUid), new RequestCallback<List<DeviceInfo>>() {
            @Override
            public void gotResult(int code, String msg, List<DeviceInfo> deviceInfo) {
                LogUtils.e(TAG, "登录极光回调---gotResult--->code: " + code + " msg: " + msg);
                if (code == 0) {
                    LogUtils.e(TAG, "极光IM登录成功");
                    ImPushUtil.getInstance().setAlias(getImUid(uid));
                    JMessageClient.registerEventReceiver(ImMessageUtil.this);
                    UserInfo myInfo = JMessageClient.getMyInfo();
                    myInfo.setNickname(AppConfig.getInstance().getUserBean().getUsername());
                    JMessageClient.updateMyInfo(UserInfo.Field.nickname, myInfo, new BasicCallback() {
                        @Override
                        public void gotResult(int i, String s) {
                            LogUtils.e(TAG, "gotResult: i:" + i + "---s:" + s);
                        }
                    });
                    ...
                    if (null != deviceInfo) {
                        for (int i = 0; i < deviceInfo.size(); i++) {
                            LogUtils.e(TAG, "deviceInfo: " + deviceInfo.get(i));
                        }
                    }
                } else {//登录不成功 尝试做一次注册
                    LogUtils.e(TAG, "未注册，用户不存在");
                    register(uid);
                }
            }
        });
    ...
    String getPwd(String id) {
        return id + PWD_SUFFIX;
    }

    //注册并登录极光IM
    public void register(String uid) {
        String imUid = getImUid(uid);
        JMessageClient.register(imUid, getPwd(imUid), new BasicCallback() {
            @Override
            public void gotResult(int code, String msg) {
                LogUtils.e(TAG, "注册极光回调---gotResult--->code: " + code + " msg: " + msg);
                if (code == 0) {
                    LogUtils.e(TAG, "极光IM注册成功");
                    loginJMessage(uid);
                } else {
                    LogUtils.e(TAG, "极光IM注册失败-->code = " + code);
                }
            }
        });
    }
    ...
    public void updateUserAvatar(File avatar, BasicCallback callback) {
    ...
    public List<ImMessageBean> getChatMessageList(String toUid) {
        ...
        for (int i = 0; i < result.size(); i++) {
            Message rawMessage = result.get(i).getRawMessage();
            if (rawMessage.getContentType() == ContentType.custom) {
                //自定义消息
                CustomContent content = (CustomContent) rawMessage.getContent();
                String customType = content.getStringValue(CUSTOM_TYPE);
                if (customType.equals(CUSTOM_TYPE_RED_PACKET)) {
                    //红包消息
                    String redId = content.getStringValue(CUSTOM_RED_PACKET_ID);
                    boolean receivced = false;
                    for (int j = result.size() - 1; j >= 0; j--) {
                        Message rawMessage2 = result.get(j).getRawMessage();
                        if (rawMessage2.getContentType() == ContentType.custom) {
                            CustomContent content2 = (CustomContent) rawMessage2.getContent();
                            String customType2 = content2.getStringValue(CUSTOM_TYPE);
                            if (customType2.equals(CUSTOM_TYPE_RED_PACKET_OPEN)) {
                                String redId2 = content2.getStringValue(CUSTOM_RED_PACKET_ID);
                                if (redId.equals(redId2)) {
                                    receivced = true;
                                    break;
                                }
                            }
                    ...
                    result.get(i).setRedOpen(receivced);
                }
            }
        }
    ...
    public String getAllUnReadMsgCount() {
        ...
        LogUtils.e(TAG, "未读消息总数----->" + unReadCount);
        String res = "";
        if (unReadCount > 99) {
            res = "99+";
        } else {
     ...
     * 通知栏点击事件
     */
    public void onEvent(NotificationClickEvent e) {
    ...
    public void onEvent(MessageEvent event) {
        ...
        if (canShow) {
            ...
            if (!AppConfig.getInstance().isFrontGround()) {
                ImNotifyMsgEvent imNotifyMsgEvent = new ImNotifyMsgEvent();
                imNotifyMsgEvent.setUid(uid);
                imNotifyMsgEvent.setUsername(event.getMessage().getFromUser().getNickname());
                imNotifyMsgEvent.setLastMessage(getMessageString(msg));
                imNotifyMsgEvent.setUnReadCount(Integer.parseInt(getAllUnReadMsgCount()));
                imNotifyMsgEvent.setLastTime(getMessageTimeString(msg));
                EventBus.getDefault().post(imNotifyMsgEvent);
            }
    ...
    public void onEvent(OfflineMessageEvent event) {
        ...
        LogUtils.e(TAG, "接收到离线消息-------->来自：" + from);
    ...
    public int getMessageType(Message msg) {
        ...
        switch (content.getContentType()) {
            ...
            case location://位置
                ...
                break;
            case custom://自定义消息
                type = ImMessageBean.TYPE_CUSTOM;
                break;
        }
        return type;
    }
    ...
    public void onEvent(ConversationRefreshEvent event) {
        ...
        String from = getAppUid(conversation);
        LogUtils.e(TAG, "接收到漫游消息-------->来自：" + from);
        if (!TextUtils.isEmpty(from) && !from.equals(AppConfig.getInstance().getUid())) {
     ...
     * @return
     */
    public ImMessageBean createCustomVideoMessage(String toUid, VideoBean videoBean) {
        String appKey = AppConfig.getInstance().getJPushAppKey();
        Map<String, String> map = new HashMap<>();
        map.put(CUSTOM_TYPE, CUSTOM_TYPE_VIDEO);
        map.put(CUSTOM_VIDEO_ID, videoBean.getId());
        map.put(CUSTOM_VIDEO_COVER, videoBean.getCoverUrl());
        map.put(CUSTOM_VIDEO_TITLE, videoBean.getTitle());
        String username = "";
        String avatar = "";
        ...
        map.put(CUSTOM_VIDEO_USERNAME, username);
        map.put(CUSTOM_VIDEO_AVATAR, avatar);
        Message message = JMessageClient.createSingleCustomMessage(PREFIX + toUid, appKey, map);
        return new ImMessageBean(AppConfig.getInstance().getUid(), message, ImMessageBean.TYPE_CUSTOM, true);
    }
     ...
     * @return
     */
    public ImMessageBean createCustomRedPacketMessage(String toUid, RedPacketNetBean bean) {
        String appKey = AppConfig.getInstance().getJPushAppKey();
        Map<String, String> map = new HashMap<>();
        map.put(CUSTOM_TYPE, CUSTOM_TYPE_RED_PACKET);
        map.put(CUSTOM_RED_PACKET_FORM, bean.getUid());
        map.put(CUSTOM_RED_PACKET_TO, bean.getToUid());
        map.put(CUSTOM_RED_PACKET_MONEY, bean.getMoney());
        map.put(CUSTOM_RED_PACKET_REMARK, bean.getRemark());
        map.put(CUSTOM_RED_PACKET_ID, bean.getId());
        Message message = JMessageClient.createSingleCustomMessage(PREFIX + toUid, appKey, map);
        return new ImMessageBean(AppConfig.getInstance().getUid(), message, ImMessageBean.TYPE_CUSTOM, true);
    }
     ...
     * @return
     */
    public ImMessageBean createCustomOpenRedPacketMessage(String toUid, RedPacketNetBean bean) {
        String appKey = AppConfig.getInstance().getJPushAppKey();
        Map<String, String> map = new HashMap<>();
        map.put(CUSTOM_TYPE, CUSTOM_TYPE_RED_PACKET_OPEN);
        map.put(CUSTOM_RED_PACKET_FORM, bean.getUid());
        map.put(CUSTOM_RED_PACKET_TO, bean.getToUid());
        map.put(CUSTOM_RED_PACKET_MONEY, bean.getMoney());
        map.put(CUSTOM_RED_PACKET_REMARK, bean.getRemark());
...
//    }
    ...
    //进入单聊
    public void enterSingleConversation(String username) {
        JMessageClient.enterSingleConversation(getImUid(username), null);
    }

    //进入群聊
    public void enterSingleConversation(long groupID) {
    ...
    //创建单聊会话
    public Conversation createSingleConversation(String username) {
        return Conversation.createSingleConversation(username, null);
    }
    ...
    //删除单个单聊会话
    public boolean deleteSingleConversation(String username) {
        return JMessageClient.deleteSingleConversation(username, null);
    }
