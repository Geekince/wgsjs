...
import com.wwsl.wgsj.adapter.RedPackAdapter;
...
import com.wwsl.wgsj.interfaces.RedPackCountDownListener;
...
public class LiveRedPackRobDialogFragment extends AbsDialogFragment implements View.OnClickListener, RedPackCountDownListener {
    ...
    private void onWin(String winCoin) {
        ...
        if (mWinGroup != null && mWinGroup.getVisibility() != View.VISIBLE) {
            ...
            WindowManager.LayoutParams params = window.getAttributes();
            params.width = DpUtil.dp2px(260);
            params.height = DpUtil.dp2px(405);
            params.gravity = Gravity.CENTER;
            params.y = -DpUtil.dp2px(35);
            window.setAttributes(params);
        ...
        if (mWinTip != null) {
            mWinTip.setText(String.format(WordUtil.getString(R.string.red_pack_16), mRedPackBean.getUserNiceName()));
        }
    }
    ...
    public void onDestroy() {
        ...
        HttpUtil.cancel(HttpConst.ROB_RED_PACK);
        if (mText != null) {
            mText.clearAnimation();
        }
        super.onDestroy();
