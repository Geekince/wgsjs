...
import com.wwsl.wgsj.activity.live.ShopWindowActivity;
import com.wwsl.wgsj.activity.me.user.UserCenterActivity;
...
public class MainMeViewHolder extends AbsMainViewHolder implements View.OnClickListener {
  ...
  private void initListener() {
    ...
    if (AppConfig.getInstance().getConfig() != null) {
      ivSetting.setVisibility(
          "1".equals(AppConfig.getInstance().getConfig().getUserCenterShow()) ? View.VISIBLE
              : View.GONE);
    }
  }
  ...
  private void setAppbarLayoutPercent() {
    appBarLayout.addOnOffsetChangedListener((appBarLayout, verticalOffset) -> {
      float percent =
          (Math.abs(verticalOffset * 1.0f) / appBarLayout.getTotalScrollRange());  //滑动比例
  ...
  private void findView() {
    ...
    ivSetting = (TextView) findViewById(R.id.ivSetting);
  ...
  private void showData() {
    ...
    ivMedal.setBackgroundResource(
        (StrUtil.isEmpty(partnerId) || "0".equals(partnerId)) ? R.mipmap.icon_medal_grey
            : R.mipmap.icon_medal);
  ...
  private void showZanDialog() {
    if (simpleCenterDialog == null) {
      simpleCenterDialog = DialogUtil.getSimpleCenterDialog(mContext, R.mipmap.icon_dz_bg, 0, 200,
          String.format("\"%s\"获得了%d个赞", curUserBean.getUsername(), curUserBean.getDzNum()));
    }
