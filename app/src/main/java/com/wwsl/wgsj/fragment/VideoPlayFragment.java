                    ...
                    if (allGranted) {
                        ...
                        mDownloadUtil.download(videoBean.getId(), AppConfig.VIDEO_PATH, Calendar.getInstance().getTimeInMillis() + ".mp4", videoBean.getVideoUrl(), new DownloadUtil.Callback() {
                            ...
                            public void onSuccess(File file) {
                                ...
                                FileUtil.saveVideo(Objects.requireNonNull(getContext()), file);
...
//        videoAdapter.setHasPadding(true);
        ...
        if (code == 0 && info != null && info.length > 0) {
            ...
            if (BuildConfig.DEBUG) {
                for (int i = 0; i < newBeans.size(); i++) {
                    LogUtils.e(TAG, "type:" + type + "--->onSuccess: " + newBeans.get(i).getTitle());
                }
            }
            ...
            if (mVideoList.size() % HttpConst.ITEM_COUNT != 0) {
                mPage = mPage > 0 ? mPage-- : 0;
            }
    ...
    public void loadData(boolean fresh) {
        switch (type) {
            ...
            case HttpConst.VIDEO_TYPE_FOLLOW:
                ...
                break;
            case HttpConst.VIDEO_TYPE_HOMETOWN://附近
                HttpUtil.getHometownVideoList(type, mPage, new HttpCallback() {
                    @Override
                    public void onSuccess(int code, String msg, String[] info) {
                        refreshBack(code, info, msg);
                    }
