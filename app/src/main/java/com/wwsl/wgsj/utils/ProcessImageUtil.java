...
public class ProcessImageUtil extends ProcessResultUtil {
            ...
            public void onSuccess(Intent intent) {
                if (mNeedCrop) {
                    ...
                    if (Build.VERSION.SDK_INT >= 24) {
                        uri = FileProvider.getUriForFile(mContext, BuildConfig.fileprovider, mCameraResult);
                    } else {
    ...
    private void crop(Uri inputUri) {
        ...
        try {
            ...
            try {
                Uri resultUri = FileProvider.getUriForFile(mContext, BuildConfig.fileprovider, mCorpResult);
                if (resultUri == null || mFragment == null || mContext == null) {
                    return;
                }
                UCrop uCrop = UCrop.of(inputUri, resultUri)
